...
export class ExplorerPanel {
    ...
    async _loadTree() {
        // Fetch projects, runtimes, environments, and documents in parallel
        const [projectsResp, runtimesResp, envsResp, docsResp] = await Promise.all([
            fetch('api/projects'),
            fetch('api/runtimes'),
            fetch('api/envs'),
            fetch('api/documents'),
        ...
        this._docsCatalog = await docsResp.json();
        ...
        // Build document tree nodes grouped by category
        const docs = this._docsCatalog.documents || [];
        ...
        const docCategoryNodes = Object.keys(docsByCategory)
            .sort()
            .map(cat => ({
                title: cat,
                key: `doccat:${cat}`,
                ...
                children: docsByCategory[cat].map(doc => ({
                    ...
                    icon: doc.location.toLowerCase().endsWith('.pdf')
    ...
    _showDetailForNode(node) {
        ...
        if (key === 'root-projects') {
            ...
            this._showDocsRootDetail();
        } else if (key.startsWith('doccat:')) {
            this._showDocCategoryDetail(key.substring(7));
        } else if (key.startsWith('doc:')) {
            const rest = key.substring(4);
            const colonIdx = rest.indexOf(':');
            this._showDocDetail(rest.substring(0, colonIdx), rest.substring(colonIdx + 1));
        }
    }
    ...
    _showDocsRootDetail() {
        ...
        // Add datalist with existing categories
        const categories = this._docsCatalog?.categories ? Object.keys(this._docsCatalog.categories) : [];
        ...
        uploadBtn.addEventListener('click', () => this._uploadDocument(nameInput, catInput, fileInput, uploadBtn, errorEl));
    ...
    _showDocCategoryDetail(category) {
        ...
        // List docs in this category
        const docs = (this._docsCatalog?.documents || []).filter(d => (d.category || 'Uncategorized') === category);
        if (docs.length === 0) {
        ...
        for (const doc of docs) {
            ...
            icon.className = doc.location.toLowerCase().endsWith('.pdf')
                ? 'fa-solid fa-file-pdf'
                : 'fa-solid fa-file-lines';
            ...
            row.addEventListener('click', () => {
                // Find and activate the tree node
    ...
    _showDocDetail(category, docName) {
        this._detailEl.innerHTML = '';
        this._addParentLabel(category);

        const doc = (this._docsCatalog?.documents || []).find(
            d => d.name === docName && (d.category || 'Uncategorized') === category
        );
        if (!doc) {
            this._detailEl.innerHTML = '<div class="explorer-detail-empty"><span>Document not found</span></div>';
            return;
        }

        const header = this._createDetailHeader(docName,
            doc.location.toLowerCase().endsWith('.pdf') ? 'fa-solid fa-file-pdf' : 'fa-solid fa-file-lines');
        this._detailEl.appendChild(header);
    ...
    async _reloadDocuments() {
        try {
            const resp = await fetch('api/documents');
            this._docsCatalog = await resp.json();
        } catch {
            this._docsCatalog = { categories: {}, documents: [] };
        }

        // Rebuild the documents subtree
        if (!this._tree) return;
        const docsRoot = this._tree.findKey('root-docs');
        if (!docsRoot) return;
        ...
        // Show the docs root detail again
        docsRoot.setActive(true, { noEvents: true });
        this._showDocsRootDetail();
        this._fireSectionChange('root-docs');
    }
