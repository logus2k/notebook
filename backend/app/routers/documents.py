...
def serve_document_file(file_path: str):
    """Serve a document file (PDF, MD, etc.)."""
    if ".." in file_path or file_path.startswith("/"):
        raise HTTPException(status_code=400, detail="Invalid file path")
    full_path = os.path.realpath(os.path.join(DOCUMENTS_DIR, "files", file_path))
    # Ensure resolved path stays within the documents directory
    docs_dir = os.path.realpath(os.path.join(DOCUMENTS_DIR, "files"))
    if not full_path.startswith(docs_dir + os.sep) and full_path != docs_dir:
